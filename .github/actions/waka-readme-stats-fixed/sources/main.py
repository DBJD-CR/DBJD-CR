...
async def get_stats() -> str:
    ...
    if EM.SHOW_UPDATED_DATE:
        ...
        # Fix: avoid duplicating the timezone label when the format string already
        # contains "UTC" (e.g. "%Y年%m月%d日 %H:%M:%S UTC+8").
        updated_date = datetime.now().strftime(EM.UPDATED_DATE_FORMAT)
        updated_date += "" if "UTC" in EM.UPDATED_DATE_FORMAT.upper() else " UTC"
