...
async def update_data_with_commit_stats(repo_details: Dict, yearly_data: Dict, date_data: Dict):
    ...
    # Fix: deduplicate commits across branches by commit oid.
    # The same commit can exist in several branches, so without this set the
    # commit would be stored in date_data multiple times and its additions /
    # deletions would be summed into yearly_data multiple times as well.
    seen_oids = set()
